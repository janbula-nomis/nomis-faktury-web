 ...
 *                    POZOR (v4.51): tohle je LIDSKÉ ROZHODNUTÍ, ne výsledek
 *                    hledání. Znamená "doklad tu být nemá", tedy hotovou
 *                    věc - v souhrnu nad seznamem (v4.50) proto padá do
 *                    tlumené věty "Vyřízeno: …" a nikdo se na něj už
 *                    nepodívá. Appka do tohohle stavu NESMÍ sama od sebe
 *                    dát žádný pohyb; když nic nenajde, patří to do
 *                    "Nespárováno" (výdaje) nebo "Příjem ke kontrole"
 *                    (příjmy). Nevracet zpátky.
 *   "Příjem ke kontrole" (od v4.51) - příchozí platba (kladná částka), ke
 *                    které appka nenašla ani nenavrhla vydanou fakturu ani
 *                    smlouvu; výchozí NEROZHODNUTÝ stav příjmů, dvojče
 *                    "Nespárováno" na příjmové straně. Počítá se mezi věci,
 *                    které ČEKAJÍ na Jana, ne mezi vyřízené. Jan
 *                    (2026-08-03): *"u příjmu v bankovních výpisech se
 *                    platby příjmy samy označí Bez dokladu, ale to je
 *                    potřeba zkontrolovat Vystavené faktury nebo
 *                    SMlouvy."* - do v4.50 appka po neúspěšném hledání
 *                    sáhla rovnou po "Bez dokladu", takže "appka nic
 *                    nenašla" a "účetní rozhodla, že doklad nemá být"
 *                    vypadalo v seznamu úplně stejně a 126 příjmů se
 *                    schovalo mezi vyřízené. Nevracet zpátky - appka nesmí
 *                    sama od sebe nastavit "Bez dokladu" žádnému pohybu.
 ...
 *
 * Import_ID (od v4.21, Jan: "ano" k nabídce appky přidat možnost smazat
 * bankovní pohyb z appky) - appka vygeneruje jedno UUID na KAŽDÉ volání
 * importu výpisu (POST bez akce) a přiřadí ho všem řádkům vzniklým v tom
 * konkrétním importu. Appka díky tomu umí smazat celý špatně naimportovaný
 * výpis najednou (DELETE ?importId=X), ne jen řádek po řádku - typicky
 * potřeba, když appka výpis naimportuje se špatně rozpoznanou měnou/
 * protistranou (viz oprava v4.20) a je potřeba ho po opravě parseru smazat
 * a naimportovat znovu. Řádky vzniklé PŘED v4.21 mají tohle pole prázdné -
 * appka je proto umí smazat jen jednotlivě (DELETE ?id=X), ne dávkově.
 */
const BANKOVNI_HEADERS = [
  ...
  'Datum_importu',
  'Smlouva_ID',
  // (v4.59) Na který řádek PŘEDPISU platba sedí (lib/predpisSchema.js).
  // Smlouva_ID zůstává - je to pořád klíč pro účetní logiku i Dashboard;
  // Predpis_ID ho jen ZPŘESŇUJE o to, za který měsíc (nebo že jde o
  // kauci) platba je. Prázdno = platba je navěšená jen na smlouvu, tak
  // jako do v4.58, a všechno funguje dál jako dřív.
  //
  // U platby rozdělené mezi víc předpisů (kauce + první nájem v jedné
  // transakci) je tu ID toho PRVNÍHO; skutečné rozdělení částky drží
  // sloupec Uhrazeno u jednotlivých předpisů. **Nedopočítávat rozdělení
  // zpětně z částky pohybu** - u sloučené platby by to nesedělo.
  'Predpis_ID',
  'Stredisko',
  'Vydana_faktura_ID',
  'Typ_dane',
