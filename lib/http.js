...
// Od v4.28 (export do Excelu, netlify/functions/export-excel.js) appka
// potřebuje odpověď s BINÁRNÍM obsahem (XLSX není textový formát jako XML) -
// appka proto tělo zapisuje jako base64 řetězec a appce (Netlify Functions)
// řekne přes `isBase64Encoded: true`, že ho má před odesláním do prohlížeče
// dekódovat zpátky na skutečné bajty souboru. `telo` appka očekává jako
// Buffer (viz lib/excelExport.js, `XLSX.write(..., { type: 'buffer' })`).
function xlsx(statusCode, telo, nazevSouboru) {
  return {
    statusCode,
    headers: {
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': 'attachment; filename="' + (nazevSouboru || 'export.xlsx') + '"',
      'Access-Control-Allow-Origin': '*',
      'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Setup-Secret',
      'Access-Control-Allow-Methods': 'GET, OPTIONS',
      'Cache-Control': 'no-store, no-cache, must-revalidate',
    ...
    body: Buffer.isBuffer(telo) ? telo.toString('base64') : telo,
  };
}

module.exports = { json, xml, xlsx };
