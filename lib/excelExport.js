 ...
 *
 * Sjednocení obsahu s Money S3 XML exportem (v4.33, Jan: "potřebuji aby
 * měli shodný obsah") - appka v4.32 rozšířila JEN XML export (Datum
 * splatnosti, DUZP, Typ_dokladu/Dobropis, KS/SS) a zapomněla na tenhle
 * paralelní Excel export, který appka udržuje ručně synchronně (appka
 * mezi nimi nesdílí žádný kód, viz export-money-s3.js/export-excel.js).
 * Od v4.33 appka do Přijatých i Vydaných faktur v Excelu přidává STEJNÁ
 * pole jako appka posílá do Money S3 XML (Datum_splatnosti, DUZP,
 * Typ_dokladu, Konstantni_symbol, Specificky_symbol), navíc u Přijatých
 * faktur appka přidává Cislo_uctu_dodavatele (appka ho v XML nemá, protože
 * je to prerekvizita jen pro QR Platbu, ale patří mezi nová pole dokladu
 * stejné dávky). Appka NEPŘIDÁVÁ derivovanou SazbaDPH1/SazbaDPH2 bucket
 * appka (appka to je legacy Money S3 konstrukce dvou sazbových bloků, ne
 * srozumitelný účetní údaj) - appka místo toho dál ukazuje existující
 * Sazba_DPH appky (jedna sazba na doklad, appka ji spravuje odjakživa).
 */
 ...
 *
 * v4.52: přibyly tři sloupce - "Účet MD", "Způsob platby" a "Platební karta".
 * Účet MD je to, kvůli čemu Jan poslal soubor Kontace.xlsx (*"tohle jsou
 * předkontace, je potřeba je zapracovat do systému"*) - účetní ho v Excelu
 * potřebuje vidět u každého dokladu. Do XML pro Money S3 se schválně
 * nepřidával: xmlFaktPrij posílá <Ucet>BAN</Ucet> (typ úhradového účtu, ne
 * nákladový) a <PredKontac>, kdežto elementy UcetMD/UcetD existují jen v
 * tvaru skladové položky, který appka nepoužívá - dopisovat je tam by byl
 * odhad. Kdyby na tom účetní trvala, chce to napřed ověřit v dokumentaci
 * Money S3, ne zkoušet naslepo.
 *
 * Prázdný "Účet MD" je legitimní stav, ne chyba exportu: Jan si vybral
 * *"Nechat prázdné a upozornit"*, takže appka účet nikdy nevymýšlí. Když je
 * ve sloupci prázdno, chybí kombinace firma+kategorie v Nastavení.
 *
 * Pozor při dalších změnách: sloupce se přidávají NA KONEC bloku, ke kterému
 * patří, ne doprostřed - účetní si na pořadí sloupců může mít navázaný
 * vlastní import.
 */
function vytvorExcelDoklady(doklady, polozkyPodleId) {
  const hlavickyDoklady = [
    'Datum dokladu', 'Datum splatnosti', 'DUZP', 'Dodavatel', 'Číslo dokladu',
    'Typ dokladu', 'Částka', 'Měna', 'DPH', 'Sazba DPH (%)', 'Kategorie',
    'Účet MD', 'Středisko', 'Firma', 'Variabilní symbol', 'Konstantní symbol',
    'Specifický symbol', 'Číslo účtu dodavatele', 'Způsob platby',
    'Platební karta', 'Stav', 'Poznámka',
  ];
  const radkyDoklady = doklady.map((d) => [
    ...
    d.Kategorie || '',
    // Účet zůstává textem (ne cisloNebo) - "518002" i "0417" musí projít tak,
    // jak jsou; číslo by ukouslo případnou vedoucí nulu.
    d.Ucet_MD || '',
    d.Stredisko || '',
    ...
    d.Konstantni_symbol || '',
    d.Specificky_symbol || '',
    d.Cislo_uctu_dodavatele || '',
    d.Zpusob_platby || '',
    // Jen poslední čtyřčíslí - celé číslo karty appka neukládá nikde, natož
    // aby ho posílala v příloze e-mailem účetní. Viz lib/platebniKartySchema.js.
    d.Platebni_karta ? `**** ${d.Platebni_karta}` : '',
    d.Stav || '',
    d.Poznamka || '',
  ]);
...
function vytvorExcelVydaneFaktury(faktury, polozkyPodleId) {
  const hlavickyFaktury = [
    'Datum vystavení', 'Datum splatnosti', 'DUZP', 'Zákazník', 'Číslo faktury',
    'Typ dokladu', 'Částka', 'Měna', 'DPH', 'Sazba DPH (%)', 'Jednotka',
    'Firma', 'Konstantní symbol', 'Specifický symbol', 'Stav', 'Datum úhrady',
    'Poznámka',
  ];
