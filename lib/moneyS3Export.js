 ...
 * (d) Rada/CisRada a další čistě Money S3 interní číslování appka nemá čím
 *     naplnit (appka žádné číselné řady nevede) - appka nastavuje jednoduché
 *     výchozí hodnoty ("PF"/"FV" + pořadové číslo v rámci exportu), Money S3
 *     při importu pravděpodobně přečísluje/doplní podle vlastního nastavení
 *     firmy. PredKontac appka od v4.32 posílá, POKUD je pro danou firmu a
 *     kategorii nastavený v novém admin listu Predkontace (Nastavení) -
 *     pokud ne (typicky dokud Jan/účetní kódy nedoplní), zůstává prázdný.
 *
 * v4.32 (viz claude/nomis-faktury-backlog.md pro plné zdůvodnění) - appka
 * doplnila reálné hodnoty tam, kde dřív posílala natvrdo placeholder:
 * Splatno (skutečné Datum_splatnosti, ne datum dokladu), KonstSym/SpecSymbol
 * (Konstantni_symbol/Specificky_symbol), Dobropis (podle Typ_dokladu),
 * DatUplDPH (nový element, z DUZP), a SazbaDPH1/SazbaDPH2 (odvozené ze
 * skutečných sazeb v položkách, ne natvrdo "21"/"12" - viz odvozenaSazba níže).
 * Bod (e) z historického seznamu už tedy neplatí.
 * ============================================================================
 */
...
// Odvodí skutečnou sazbu DPH použitou v dané "skupině" položek (viz
// sazbaSkupina výš) ze souhrnu appka už správně počítá (zaklad/dph) - místo
// dřívějšího natvrdo "21"/"12" appka od v4.32 sazbu dopočítá zpětně z
// poměru dph/zaklad, zaokrouhleno na celé procento. Pokud appka v dané
// skupině žádné položky nemá (zaklad === 0), sazba je pro export
// irelevantní - appka vrátí vychozi hodnotu (Money S3 formát pole i tak
// vyžaduje, appka ho neponechá prázdný).
function odvozenaSazba(zaklad, dph, vychozi) {
  if (zaklad > 0) {
    const s = Math.round((dph / zaklad) * 100);
    if (Number.isFinite(s) && s > 0) return String(s);
  }
  return vychozi;
}
...
function xmlFaktPrij(doklad, polozky, mojeFirma, cisRada, predkontaceMapa) {
  ...
  const jeDobropis = doklad.Typ_dokladu === 'Dobropis';
  const firmaDokladu = doklad.Firma_potvrzena || doklad.Firma_AI_odhad || '';
  const predKontac = (predkontaceMapa && predkontaceMapa[firmaDokladu + '|' + (doklad.Kategorie || '')]) || '';
  ...
  return (
    ...
    el('DatUcPr', datum) +
    el('PlnenoDPH', datum) +
    el('DatUplDPH', doklad.DUZP || datum) +
    el('Splatno', doklad.Datum_splatnosti || datum) +
    el('Doruceno', datum) +
    el('DatSkPoh', datum) +
    el('KonstSym', doklad.Konstantni_symbol || '') +
    el('KodDPH', '19Ř40,41') +
    el('VarSymbol', doklad.Variabilni_symbol || '') +
    el('SpecSymbol', doklad.Specificky_symbol || '') +
    el('Ucet', 'BAN') +
    el('Druh', 'N') +
    el('Dobropis', jeDobropis ? '1' : '0') +
    el('PredKontac', predKontac) +
    el('ZpVypDPH', '1') +
    el('SazbaDPH1', odvozenaSazba(souhrn.zaklad22, souhrn.dph22, '21')) +
    el('SazbaDPH2', odvozenaSazba(souhrn.zaklad5, souhrn.dph5, '12')) +
    xmlSouhrnDph(souhrn) +
...
function xmlFaktVyd(faktura, polozky, mojeFirma, cisRada) {
  ...
  const jeDobropis = faktura.Typ_dokladu === 'Dobropis';
  ...
  return (
    ...
    el('DatUcPr', datum) +
    el('PlnenoDPH', datum) +
    el('DatUplDPH', faktura.DUZP || datum) +
    el('Splatno', faktura.Datum_splatnosti || datum) +
    el('DatSkPoh', datum) +
    el('KonstSym', faktura.Konstantni_symbol || '') +
    el('KodDPH', '19Ř01,02') +
    el('VarSymbol', faktura.Cislo_faktury || '') +
    el('SpecSymbol', faktura.Specificky_symbol || '') +
    el('Ucet', 'BAN') +
    el('Druh', 'N') +
    el('Dobropis', jeDobropis ? '1' : '0') +
    el('Uhrada', 'převodem') +
...
// netlify/functions/export-money-s3.js). `predkontaceMapa` (nepovinná) je
// mapa "Firma|Kategorie" -> kód předkontace (appka ji sestaví z nového
// admin listu Predkontace, viz netlify/functions/predkontace.js) - appka
// pro doklady, kde kombinace není nastavená, posílá prázdné PredKontac
// (viz docblock výš, bod d).
function vytvorXmlPrijateFaktury(doklady, polozkyPodleId, firma, ted, predkontaceMapa) {
  const casovaZnacka = ted || new Date();
  const rok = casovaZnacka.getFullYear();
  const telaFaktur = doklady
    .map((d, idx) => xmlFaktPrij(d, (polozkyPodleId && polozkyPodleId[d.ID]) || [], firma, idx + 1, predkontaceMapa))
    .join('');
