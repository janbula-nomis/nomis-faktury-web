...
//
// Pozn. k záložním modelům: pokud je primární model dlouhodobě přetížený
// (Google vrací 503 "high demand" opakovaně na víc dokladech po sobě, ne
// jen na jednom dokladu jednorázově), appka to nezkouší dokola na tom
// samém modelu, ale přepne na jiný ("záložní") model - ty mají typicky
// oddělenou kapacitu, takže bývají dostupné, i když je hlavní model
// zrovna přetížený.
const GEMINI_MODEL = process.env.GEMINI_MODEL || 'gemini-flash-latest';
const ZALOZNI_MODELY = ['gemini-2.5-flash-lite', 'gemini-2.5-flash'];
...
// se opakováním nevyřešily). Držíme málo pokusů s krátkým čekáním na
// jeden model, ať appka zbytečně nenaráží na časový limit Netlify
// Functions (viz README-DEPLOY.md - synchronní funkce mají tvrdý limit
// a appka navíc může narazit na kratší limit "brány" před funkcí).
const OPAKOVATELNE_KODY = [429, 500, 502, 503, 504];
const POKUSU_NA_MODEL = 2;
const CEKANI_MS = 1000;

function pockej(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

// Zavolá jeden konkrétní model s pár rychlými pokusy. Vrátí response, nebo
// vyhodí Error, pokud selžou všechny pokusy na tomhle modelu.
async function zavolejModel(model, payload, apiKlic) {
  let posledniChyba;
  for (let pokus = 1; pokus <= POKUSU_NA_MODEL; pokus += 1) {
    ...
    const maSmyslOpakovat = OPAKOVATELNE_KODY.includes(response.status) && pokus < POKUSU_NA_MODEL;
    if (!maSmyslOpakovat) throw posledniChyba;

    await pockej(CEKANI_MS);
  }
  throw posledniChyba;
}
