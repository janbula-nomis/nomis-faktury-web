...
const SMLOUVY_HEADERS = [
  ...
  'Kauce_splatnost',
  /*
   * (v4.81) Podklady pro TIŠTĚNOU NÁJEMNÍ SMLOUVU.
   *
   * Jan 2026-08-21: *„potřebuji generovat dokumentaci, po vyplnění karty
   * udělat tiskový výstup k podpisu"*. Většina textu smlouvy je pevná (viz
   * public/dokumenty.js); tohle je to, co se v jeho čtyřech vzorech
   * lišilo kus od kusu a appka to zatím nevedla.
   *
   * Najemce_ID - odkaz do listu Najemci (v4.81). Do v4.80 uměla smlouva
   *   o druhé straně jen `Druha_strana`, což je JMÉNO - do smlouvy je ale
   *   potřeba i IČ, sídlo a kdo za společnost jedná. Jan si ve volbě vybral
   *   číselník místo polí na smlouvě, protože Schulte Group má u něj víc
   *   bytů. **`Druha_strana` zůstává** a nikam se neztrácí: je to text,
   *   podle kterého appka odjakživa páruje platby a kreslí odznaky na kartě
   *   bytu. Najemce_ID ho jen zpřesňuje, stejně jako Najemni_jednotka_ID
   *   zpřesňuje Stredisko.
   * Datum_uzavreni - kdy se smlouva podepsala. NENÍ to `Platnost_od`:
   *   v Janových vzorech se podepisovalo i tři měsíce před nastěhováním
   *   (Kresiak: smlouva 16. 12. 2024, protokol až později). Předávací
   *   protokol na tenhle údaj odkazuje větou „Strany potvrzují, že spolu
   *   dne … uzavřely nájemní smlouvu" - do v4.80 tam appka nechávala
   *   prázdné pole, protože to datum neměla kde vzít.
   * Najem_rozpis - volný text, když se nájem skládá z víc částí: *„28 700 Kč
   *   resp. 2 300 Kč; tj. celkem 31 000 Kč"* (byt + garáž v suterénu).
   *   Prázdné = nájem je jedno číslo. Appka to nerozpadá na položky -
   *   ve smlouvě je to jedna věta a jako věta se i zapisuje.
   * Zalohy_pausalni - 'ANO', když jsou zálohy paušál a NEBUDE se vyúčtovávat.
   *   Ve smlouvě je to celá věta navíc a je to rozdíl, na kterém stojí
   *   peníze: u paušálu nájemník nedostane přeplatek ani nedoplatí.
   *   Prázdné znamená „nevyplněno", ne „vyúčtuje se" - dokud to Jan
   *   neoznačí, smlouva tu větu netiskne ani jednu.
   * Inflace_od - ke kterému datu se poprvé smí zvednout nájem o inflaci.
   *   V obou Janových novějších vzorech to je 1. 1. roku následujícího po
   *   začátku nájmu, ale **appka to nedopočítává** - datum, které jde do
   *   podepisované smlouvy, si má člověk potvrdit. Prázdné = ve smlouvě
   *   zůstane pole k vyplnění.
   * Pocet_osob, Osoby - kolik lidí smí v bytě bydlet a kdo to jmenovitě je
   *   (starší smlouva Schulte TZB: *„Společně s nájemcem mohou Byt užívat
   *   tyto osoby: Jan Tapaj, Martin Sacha… Přiměřený počet pro Byt jsou 3"*).
   *   Nepovinné - když jsou prázdné, smlouva ten odstavec vynechá.
   */
  'Najemce_ID',
  /*
   * (v4.83) Pronajimatel_ID - kdo je u téhle smlouvy pronajímatel (odkaz do
   * listu Pronajimatele). Do v4.82 si ho člověk vybíral v roletce pokaždé,
   * když se dokument tiskl, a nikde se neuložil.
   *
   * Dodatek to ale potřebuje vědět sám: Janův dodatek č. 2 mění BANKOVNÍ
   * ÚČET, a ten je na pronajímateli, ne na smlouvě. Bez tohohle odkazu by
   * appka nevěděla, čí účet má změnit.
   *
   * Prázdné = nevybráno. Appka pak zkusí pronajímatele odvodit podle
   * `Firma`, ale je to jen NÁPOVĚDA (viz vychoziPronajimatel v
   * public/dokumenty.js) - u dodatku, který mění cizí údaj, se s nápovědou
   * nespokojí a řekne, že není co změnit.
   */
  'Pronajimatel_ID',
  'Datum_uzavreni',
  'Najem_rozpis',
  ...
  'Osoby',
  /*
   * (v4.85) ROZPIS ZÁLOH A OZNAČENÍ PLATBY - podklady pro PLATEBNÍ VÝMĚR.
   *
   * Jan 2026-08-21: *„obsahuje kauci, nájem, zálohy služby SVJ, energie,
   * internet, ostatní (…) variabilní symbol např. 45_695 nebo 01a_2236"*.
   *
   * Zaloha_svj / Zaloha_energie / Zaloha_internet / Zaloha_ostatni -
   *   rozpis toho, co je dneska v jednom čísle `Zaloha_na_sluzby`.
   *   **`Zaloha_na_sluzby` ZŮSTÁVÁ a je pořád ta závazná částka**: na ní
   *   stojí předpis plateb i kontrola úhrad, a ta se kvůli hezčí příloze
   *   měnit nesmí. Rozpis je popis téhož čísla, ne jeho náhrada.
   *
   *   Když je rozpis vyplněný, výměr tiskne sloupce podle něj; když není,
   *   tiskne jeden sloupec „Zálohy na služby". Appka rozpis NEDOPOČÍTÁVÁ
   *   ani nedoplňuje zbytek do „Ostatní" - rozdělit 4 500 Kč mezi SVJ,
   *   energie a internet umí jenom člověk, který ví, co v nich je.
   *
   *   Když se rozpis nerovná `Zaloha_na_sluzby`, appka to při tisku výměru
   *   napíše. Nezvolí si, které číslo je to pravé - to je otázka na Jana.
   *
   * Oznaceni_platby - Janův čitelný tvar „01a_2236" nebo „45_695".
   *   **NENÍ to variabilní symbol.** Ten smí být podle bankovního
   *   standardu jen celé číslo o nejvýš deseti číslicích, takže
   *   podtržítko ani písmeno v něm být nemůže (Jan se 2026-08-21 ptal,
   *   jestli to jde do banky - nejde). Označení jde do zprávy pro
   *   příjemce a tiskne se na výměr, aby nájemník poznal, co platí;
   *   párování drží `Variabilni_symbol`.
   *
   *   Když je `Variabilni_symbol` prázdný, výměr si ho z označení
   *   dopočítá (viz vsZOznaceni v public/qrKod.js: jednotka na tři
   *   číslice, kde písmeno je pořadí, dům na čtyři). Dopočet se ale
   *   NEZAPISUJE do tabulky - do ní patří číslo, které Jan potvrdil.
   */
  'Zaloha_svj',
  'Zaloha_energie',
  'Zaloha_internet',
