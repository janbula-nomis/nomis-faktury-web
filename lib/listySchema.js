...
const { DODATKY_HEADERS, DODATKY_ZMENY_HEADERS } = require('./dodatkySchema');
const { FIRMY_REJSTRIK, OSOBA_JAN_BULA } = require('./rejstrikFirem');

/*
 * Ukázkové řádky listu „Pronajimatele" - Janovy tři firmy a on sám jako
 * fyzická osoba.
 *
 * Skládá se to z lib/rejstrikFirem.js, ne ručně opsané do pole, aby
 * existoval JEDEN zdroj těch čísel. Kdyby tu byla druhá kopie IČ a účtů,
 * rozešly by se - přesně tak, jak se rozešel seznam listů, než se v v4.75
 * vytáhl ze setup.js sem (viz hlavička nahoře).
 *
 * ID jsou stabilní texty, ne náhodná UUID. Ukázka se sype jen do úplně
 * prázdného listu, takže se nemá s čím srazit, a stabilní ID znamená, že
 * dvojí spuštění setupu vypadá stejně.
 */
function ukazkaPronajimatelu() {
  const radky = FIRMY_REJSTRIK.map((f) => ({
    ID: 'pronajimatel-' + f.ICO,
    Nazev: f.obchodniFirma,
    Druh: 'Firma',
    Firma: f.nazev,
    ICO: f.ICO,
    DIC: f.DIC,
    Spisova_znacka: f.spisovaZnacka,
    Sidlo: f.sidlo,
    Zastoupena: f.jednatel,
    Datum_narozeni: '',
    Bankovni_ucet: f.ucet,
    Email: OSOBA_JAN_BULA.email,
    Telefon: OSOBA_JAN_BULA.telefon,
    // Účet appka opisuje z podepsané smlouvy, ne z rejstříku - řekne to
    // rovnou v poznámce, ať to Jan v tabulce vidí bez hledání.
    Poznamka: [f.poznamka, f.ucetZdroj ? 'Číslo účtu podle: ' + f.ucetZdroj : '']
      .filter(Boolean).join(' '),
  }));

  radky.push({
    ID: 'pronajimatel-osoba-jan-bula',
    Nazev: OSOBA_JAN_BULA.nazev,
    Druh: 'Osoba',
    Firma: '',
    ICO: '', DIC: '', Spisova_znacka: '',
    Sidlo: OSOBA_JAN_BULA.adresa,
    Zastoupena: '',
    Datum_narozeni: OSOBA_JAN_BULA.datumNarozeni,
    Bankovni_ucet: OSOBA_JAN_BULA.ucet,
    Email: OSOBA_JAN_BULA.email,
    Telefon: OSOBA_JAN_BULA.telefon,
    Poznamka: 'Číslo účtu podle: ' + OSOBA_JAN_BULA.ucetZdroj,
  });

  return radky.map((r) => PRONAJIMATELE_HEADERS.map((h) => (r[h] !== undefined ? r[h] : '')));
}
...
const LISTY = [
  ...
  {
    // Dodatky k nájemním smlouvám (od v4.83) - hlavička dokumentu.
    // Proč jsou změny ve vlastním listu a ne volným textem, vysvětluje
    // lib/dodatkySchema.js.
    nazev: 'Dodatky',
    hlavicky: DODATKY_HEADERS,
    ukazka: [],
  },
  {
    // Řádky změn jednoho dodatku (1 dodatek : N změn), stejný vzor jako
    // Meridla : Meridla_Odecty.
    nazev: 'Dodatky_Zmeny',
    hlavicky: DODATKY_ZMENY_HEADERS,
    ukazka: [],
  },
  {
    // Číselník nájemců (od v4.81) - druhá strana smlouvy. Stejné sloupce
    // jako Pronajimatele; proč jsou to přesto dva listy, je v hlavičce
    // lib/smluvniStrany.js. Ukázka je prázdná - nájemce si Jan zapíše sám,
    // appka o nich nic nemá odkud vědět.
    nazev: 'Najemci',
    hlavicky: PRONAJIMATELE_HEADERS,
    ukazka: [],
  },
  {
    // Hlavičky smluv a předávacích protokolů (od v4.80) - viz
    // lib/pronajimateleSchema.js, proč to není v listu Firmy.
    //
    // Ukázka je PŘEDVYPLNĚNÁ ověřenými údaji z obchodního rejstříku - Jan
    // 2026-08-21: *„vyplň údaje všech mých firem do databáze, najdi si to
    // v OR"*. Odkud který údaj je (a proč čísla účtů nejsou z rejstříku),
    // stojí v hlavičce lib/rejstrikFirem.js.
    //
    // Vychozi zůstává u všech PRÁZDNÉ. Předvybrat, za koho se smlouva
    // vytiskne, je jediné rozhodnutí v tomhle listu, které appce nepřísluší.
    nazev: 'Pronajimatele',
    hlavicky: PRONAJIMATELE_HEADERS,
    ukazka: ukazkaPronajimatelu(),
  },
