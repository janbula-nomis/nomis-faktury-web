 ...
 * JMÉNA ZÁKAZNÍKA, záměrně NE podle variabilního symbolu - zákazníci VS
 * často nevyplní správně/vůbec, takže by to bylo nespolehlivé. Appka i tady
 * jen NAVRHUJE (nikdy sama nepotvrzuje), stejná filozofie jako navrhniShodu.
 *
...
function navrhniShoduPrijem(pohyb, kandidatiFaktury) {
  ...
  kandidatiFaktury.forEach((f) => {
    ...
    // (v3.23) Shoda jména je teď VŽDY povinná - u plné i částečné shody
    // částky. Samotná shoda čísel appce k návrhu nestačí, viz komentář výš.
    if (!shodaNazvu) return;
    ...
    let skore = 1 + 2; // částka sedí (základ) + shoda jména (teď vždy splněna)
...
/**
 * Zkusí k PŘÍCHOZÍ platbě (kladná částka) najít nejlepší odpovídající
 * nájemní Smlouvu (Typ === "Nájem") ze seznamu kandidátů (od v4.19, Jan:
 * "příjmy z nájmu přiřadit k bankovním vypisům, zdrojem jsou nájemní
 * smlouvy, které načtu do registru smluv") - na rozdíl od
 * jePodobnaShodaSmlouvy výš appka tu porovnává pohyb PŘÍMO proti údajům
 * samotné smlouvy (Druha_strana = nájemce, Ocekavana_castka), BEZ nutnosti
 * napřed ručně přiřadit nějaký "vzorový" pohyb - appka tak umí navrhnout
 * spárování hned u PRVNÍ platby od nájemce, jakmile Jan smlouvu založí.
 *
 * Shoda jména nájemce (normalizovaný `Druha_strana`) je VŽDY povinná -
 * stejná filozofie jako u cizoměnových dokladů/navrhniShoduPrijem výš,
 * samotná podobná částka by u víc nájemních smluv se stejným/podobným
 * nájemným byla slabý/nespolehlivý signál. Appka měnu porovnává stejně
...
function navrhniShoduNajem(pohyb, kandidatiSmlouvy) {
  ...
  kandidatiSmlouvy.forEach((s) => {
    if (String(s.Typ || '').trim() !== 'Nájem') return;
    ...
    const skore = 1 + 2; // částka odpovídá (v toleranci) + shoda jména (povinná)
    if (skore > nejlepsiSkore) {
      nejlepsiSkore = skore;
      nejlepsi = s;
    }
