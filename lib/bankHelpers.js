 ...
 *     zaplatil jen část) - appka ji nabídne JEN, když navíc sedí i jméno
 *     zákazníka (samotné "nižší částka" je příliš slabý/nespolehlivý signál
 *     bez dalšího potvrzení - jinak by se do faktury mohla omylem navrhnout
 *     jakákoli menší platba). Appka pak appce (banka.js) řekne `castecne:
 *     true`, ať appka ví, jestli po potvrzení nastavit Vydane_faktury.Stav
 *     na "Uhrazeno" nebo "Částečně uhrazeno".
 *
...
function navrhniShoduPrijem(pohyb, kandidatiFaktury) {
  ...
  kandidatiFaktury.forEach((f) => {
    ...
    if (castecnaShoda && !shodaNazvu) return; // viz komentář výš - částečná shoda vyžaduje i jméno

    let skore = 1; // částka (plná, nebo přijatelná částečná se jménem) sedí - základ
    if (shodaNazvu) skore += 2;
