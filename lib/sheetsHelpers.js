...
// (v4.71) Každé volání Sheets API jde přes opakování při nárazu na
// minutový limit - viz lib/opakuj.js pro důvod (Jan nahrál 30 dokladů
// najednou a deset spadlo na „Quota exceeded … Read requests per minute").
const { opakujPriLimitu } = require('./opakuj');
...
/*
 * (v4.72) Která pole se do listu NEZAPÍŠOU, protože v něm takový sloupec
 * není.
 *
 * Přeskakování neznámých sloupců je záměr (viz hlavička souboru): zapsat
 * hodnotu „někam" by bylo horší než ji nezapsat. Problém je, že to bylo
 * DOCELA TICHÉ - appka vrátila „uloženo" a hodnota se ztratila. Přesně na
 * to narazil Jan u zaškrtávátka „Zaúčtováno": sloupec v jeho tabulce ještě
 * nebyl (nespustil `/api/setup` po v4.63), zaškrtnutí vypadalo, že prošlo,
 * a po načtení stránky bylo pryč.
 *
 * Funkce proto vrátí seznam vynechaných polí a volající se může rozhodnout,
 * jestli je to drobnost, nebo důvod říct chybu. Prázdné hodnoty se
 * nehlásí - nezapsat prázdno nikomu nevadí.
 */
function vynechanaPole(skutecneHlavicky, rowObj) {
  const zname = new Set(skutecneHlavicky);
  return Object.keys(rowObj || {}).filter((klic) => {
    if (klic === '_row' || zname.has(klic)) return false;
    const hodnota = rowObj[klic];
...
async function nactiSkutecneHlavicky(sheets, spreadsheetId, sheetName, zalozniHlavicky) {
  let res;
  try {
    res = await opakujPriLimitu(() => sheets.spreadsheets.values.get({ spreadsheetId, range: sheetName + '!1:1' }));
  } catch (e) {
...
async function readSheetObjects(sheets, spreadsheetId, sheetName) {
  let res;
  try {
    res = await opakujPriLimitu(() => sheets.spreadsheets.values.get({ spreadsheetId, range: sheetName }));
  } catch (e) {
    throw zabalChybuChybejicihoListu(e, sheetName);
  }
...
async function appendRow(sheets, spreadsheetId, sheetName, headers, rowObj) {
  ...
  try {
    await opakujPriLimitu(() => sheets.spreadsheets.values.append({
      spreadsheetId,
      range: sheetName,
  ...
  return { vynechano: vynechanaPole(skutecneHlavicky, rowObj) };
}
...
async function updateRow(sheets, spreadsheetId, sheetName, headers, rowNumber, rowObj) {
  ...
  // (v4.72) Volající se může podívat, co se do listu nevešlo - viz
  // vynechanaPole() výš. Kdo návratovou hodnotu ignoruje, chová se stejně
  // jako dřív.
  return { vynechano: vynechanaPole(skutecneHlavicky, rowObj) };
}
